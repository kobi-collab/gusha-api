...
// ─── Auth event system ───
// Allows useAuth to be notified when login/logout happens
...
/**
 * Complete login: stores token + user info and notifies all listeners.
 * This is the single entry point for completing a login on native.
 */
export async function completeLogin(sessionToken: string, user: User): Promise<void> {
  console.log("[Auth] completeLogin called for user:", user.name);
  await setSessionToken(sessionToken);
  await setUserInfo(user);
  // Notify useAuth hooks that auth state has changed
  notifyAuthChange();
  console.log("[Auth] completeLogin done, listeners notified");
}

/**
 * Complete logout: clears token + user info and notifies all listeners.
 */
export async function completeLogout(): Promise<void> {
  console.log("[Auth] completeLogout called");
  await removeSessionToken();
  await clearUserInfo();
  notifyAuthChange();
  console.log("[Auth] completeLogout done, listeners notified");
}
